...
do {
    ...
    check(source.contains("perfRun(durationMs)"), "bridge exposes single-call performance sampler")
    check(source.contains("waitForContentStable(timeoutMs, quietMs)"), "bridge exposes observer-backed stability wait")
    check(source.contains("configure(nextSelectors)"), "bridge exposes runtime selector configuration")
    ...
    check(source.contains("conversationScroller"), "bridge reports inner conversation scroller")
    check(source.contains("contentState"), "bridge reports page content state")
    check(source.contains("longTasks"), "bridge reports long-task capability")
